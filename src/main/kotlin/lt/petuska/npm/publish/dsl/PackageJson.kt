...
open class JsonObject<T>(initialConfig: Map<String, T?> = emptyMap()) :
    MutableMap<String, T?> by initialConfig.toMutableMap(), Serializable {

  operator fun plus(other: JsonObject<T>) =
      JsonObject<T>(mapOf(*(this.entries + other.entries).toTypedArray())).also {TODO()}
  ...
  /** Assigns a json value as ` [this]= [value]` if it's not already set */
  infix fun String.ifNotSet(value: T?) {
    if (!this@JsonObject.containsKey(this)) {
      this@JsonObject[this] = value
    }
  }
...
fun <T : JsonObject<*>> T.writeTo(packageJsonFile: File) =
    this.also {
      ...
      packageJsonFile.writer().use { JsonObject.gson.toJson(this, it) }
    }
 ...
 */
class PackageJson(initialConfig: Map<String, Any?>, config: PackageJson.() -> Unit = {}) :
    JsonObject<Any>(initialConfig) {
  init {
    this.apply(config)
  }
  ...
  constructor(
      ...
      mapOf(Pair("name", npmFullName(name, scope)), Pair("version", version), Pair("scope", scope)),
      config)
  ...
  /** [author](https://docs.npmjs.com/files/package.json#people-fields-author-contributors) */
  fun author(config: Person.() -> Unit = {}) =
      (author ?: Person()).apply(config).also { author = it }
  ...
  fun directories(config: Directories.() -> Unit = {}) =
      (directories ?: Directories()).apply(config).also { directories = it }
  ...
  fun repository(config: Repository.() -> Unit = {}) =
      (repository ?: Repository()).apply(config).also { repository = it }
  ...
  fun devDependencies(config: JsonObject<String>.() -> Unit = {}) =
      (devDependencies ?: JsonObject()).apply(config).also { devDependencies = it }
  ...
  fun peerDependencies(config: JsonObject<String>.() -> Unit = {}) =
      (peerDependencies ?: JsonObject()).apply(config).also { peerDependencies = it }
  ...
  fun optionalDependencies(config: JsonObject<String>.() -> Unit = {}) =
      (optionalDependencies ?: JsonObject()).apply(config).also { optionalDependencies = it }
  ...
  fun publishConfig(config: PublishConfig.() -> Unit = {}) =
      (publishConfig ?: PublishConfig()).apply(config).also { publishConfig = it }
